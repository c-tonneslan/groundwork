...
- **Ask in plain English.** A natural-language box turns a question into one of three intents via an LLM: *filter* the map (*"large new construction since 2020"*), *answer* a question with a **cited number** (*"how many units in Brooklyn since 2020?"* → the total plus the projects it's drawn from), or **refuse** when the question needs something the data can't express (transit proximity, rent burden, bedroom mix, price). It's a two-stage router: a fast **deterministic parser** handles the common shapes offline (a borough name, "new construction", "since 2020", "over 100 units", "how many units in X") with **no API key and no cost**, and only the long tail escalates to the LLM. Either way it's constrained to a fixed schema and every value is whitelisted against the active city's real vocabulary — it never writes SQL; answers run as fixed parameterized aggregates. The endpoint is rate-limited and caches repeat questions. `ANTHROPIC_API_KEY` (optionally `ANTHROPIC_MODEL`) unlocks free-form questions; without it, the parser still covers the common cases. The translation core is unit-tested (`npm test`, no key needed) and has a live accuracy scorecard (`npm run eval:ask`, needs the key).
