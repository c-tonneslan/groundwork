...
- **Ask in plain English.** A natural-language box turns a question into one of three intents via an LLM: *filter* the map (*"large new construction since 2020"*), *answer* a question with a **cited number** (*"how many units in Brooklyn since 2020?"* → the total plus the projects it's drawn from), or **refuse** when the question needs something the data can't express (transit proximity, rent burden, bedroom mix, price). The model is constrained to a fixed schema and every value is whitelisted against the active city's real vocabulary — it never writes SQL; answers run as fixed parameterized aggregates. The endpoint is rate-limited and caches repeat questions. Requires `ANTHROPIC_API_KEY` (optionally `ANTHROPIC_MODEL`); degrades to a clear message when unset. The translation core is unit-tested (`npm test`, no key needed) and has a live accuracy scorecard (`npm run eval:ask`, needs the key).
